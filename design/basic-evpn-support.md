...
1. **ImportRTs**: Import route targets for the same VNI are aggregated from all FRRConfigurations and duplicates are removed. However, if one FRRConfiguration omits import RTs for a VNI (relying on FRR's automatic defaults) while another explicitly specifies them, the configuration is invalid — mixing implicit and explicit route targets is ambiguous.
2. **ExportRTs**: Export route targets for the same VNI are aggregated from all FRRConfigurations and duplicates are removed. The same implicit/explicit ambiguity rule as ImportRTs applies.
...
- One FRRConfiguration can contribute specific L2VNI customizations with RD/RT values
- Another FRRConfiguration can add additional route targets to existing VNIs
...
4. **Route target consistency**: For the same VNI across multiple FRRConfigurations, either all configurations must omit route targets (relying on FRR defaults) or all must explicitly specify them. Mixing omitted and explicit route targets for the same VNI is invalid because the intended result is ambiguous
