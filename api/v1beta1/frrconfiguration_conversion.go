...
func (src *FRRConfiguration) ConvertTo(dstRaw conversion.Hub) error {
	...
	log.Printf("ConvertTo: Converting CronJob from Spoke version v1 to Hub version v2;"+
...
func (dst *FRRConfiguration) ConvertFrom(srcRaw conversion.Hub) error {
	...
	log.Printf("ConvertFrom: Converting CronJob from Hub version v2 to Spoke version v1;"+
		"source: %s/%s, target: %s/%s", src.Namespace, src.Name, dst.Namespace, dst.Name)
