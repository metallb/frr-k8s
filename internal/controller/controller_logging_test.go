...
var _ = Describe("Frrk8s logging", Serial, func() {
	Context("when the FRRConfiguration and FRRState controller reconcilers are triggered", func() {
		...
		AfterEach(func() {
			...
			frrOperToDel := &v1beta1.FRROperatorConfiguration{}
			...
			Expect(err).ToNot(HaveOccurred())
			Eventually(func() int {
				frrConfigList := &v1beta1.FRROperatorConfigurationList{}
				err := k8sClient.List(context.Background(), frrConfigList)
				Expect(err).ToNot(HaveOccurred())
				return len(frrConfigList.Items)
			}).Should(Equal(0))
		...
		DescribeTable("should log at the correct log level", func(logLevel logging.Level) {
			...
			// Create the FRROperatorConfiguration with the desired log level.
			frrOperatorConfig := &v1beta1.FRROperatorConfiguration{
				ObjectMeta: ctrl.ObjectMeta{
					Name:      FRROperatorConfigurationName,
					Namespace: testNamespace,
				},
				Spec: v1beta1.FRROperatorConfigurationSpec{
					LogLevel: string(logLevel),
				},
			}
			err := k8sClient.Create(context.Background(), frrOperatorConfig)
			Expect(err).ToNot(HaveOccurred())
