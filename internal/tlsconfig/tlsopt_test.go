...
func TestParseCurvePreferences(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "unsupported curve ID returns error",
			input:   "99999",
			wantErr: true,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Fatalf("parseCurvePreferences(%q) error = %v, wantErr %v", tt.input, err, tt.wantErr)
			}
			if !tt.wantErr {
				if !equalCurveIDs(got, tt.want) {
...
func TestParseTLSVersion(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Fatalf("parseTLSVersion(%q) error = %v, wantErr %v", tt.input, err, tt.wantErr)
			}
...
func TestOptFor(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !equalUint16s(cfg.CipherSuites, tc.wantCipherSuites) {
				t.Errorf("CipherSuites = %v, want %v", cfg.CipherSuites, tc.wantCipherSuites)
			}
			if !equalCurveIDs(cfg.CurvePreferences, tc.wantCurvePreferences) {
...
func equalCurveIDs(a, b []tls.CurveID) bool {
	if len(a) != len(b) {
		return false
	}
	for i := range a {
