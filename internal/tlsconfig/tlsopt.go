...
// parseCurvePreferences parses a comma-separated list of numeric CurveID values.
...
func parseCurvePreferences(curves string) ([]tls.CurveID, error) {
	...
	for _, p := range parts {
		...
		if err != nil {
			return nil, fmt.Errorf("invalid curve preference %q: must be a numeric CurveID", p)
		}
		id := int32(n)
