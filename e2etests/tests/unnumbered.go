...
var _ = ginkgo.Describe("Unnumbered", func() {
	...
	ginkgo.DescribeTable("BGP session is established and routes are verified", func(rc frrconfig.RouterConfigUnnumbered) {
		...
		ginkgo.By("validating the node and p2p conatiner peered")
...
router bgp %[2]d
 ...
 neighbor %[1]s interface remote-as %[2]d
 neighbor %[1]s bfd
 address-family ipv6 unicast
  neighbor %[1]s activate
exit-address-family
...
`
			cr := frrk8sv1beta1.FRRConfiguration{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "raw-for-" + externalP2PContainer.Name,
					Namespace: k8s.FRRK8sNamespace,
				},
				Spec: frrk8sv1beta1.FRRConfigurationSpec{
					NodeSelector: metav1.LabelSelector{
					...
					Raw: frrk8sv1beta1.RawConfig{
						Config:   fmt.Sprintf(raw, iface, infra.FRRK8sASN),
						Priority: 5,
					},
				},
			}
			...
			nodeP2PContainer := executor.ForContainer(nodeWithP2PConnection.Name)
			lla, err := netdev.LinkLocalAddressForDevice(nodeP2PContainer, iface)
			Expect(err).NotTo(HaveOccurred())
			validateUnnumberedBGPPeering(externalP2PContainer, lla)
		})
	})
})
...
func validateUnnumberedBGPPeering(peer *frrcontainer.FRR, nodeLLA string) {
	...
	Eventually(func() error {
		...
		for _, n := range neighbors {
			if n.HostForeign == nodeLLA && n.Connected && n.BFDInfo.Status == "Up" {
				return nil
			}
		}
