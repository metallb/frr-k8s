...
func init() {
	flag.StringVar(&defaultLogLevel, "default-log-level", "debug", "Default log level to test against when no configuration is present")
}
	...
	ginkgo.Context("for status cleaner", func() {
		...
		ginkgo.When("FRRK8sConfiguration changes", func() {
			ginkgo.It("logs with the correct log levels", func() {
				...
				checkStatusCleanerLogs(cs, frrk8sStatusCleanerPods, defaultLogLevel, beforeUpdateTime)
	...
	ginkgo.Context("for frr-k8s", func() {
		ginkgo.When("no FRRK8sConfiguration is present", func() {
			ginkgo.It("logs with default log level", func() {
				...
				Expect(err).NotTo(HaveOccurred())
				checkFRRK8sLogs(cs, frrk8sPods, beforeUpdateTime, defaultLogLevel)
				checkBGPStateLogs(cs, frrk8sPods, beforeUpdateTime, defaultLogLevel)
			})
		})

		ginkgo.When("FRRK8sConfiguration with empty spec is added", func() {
			ginkgo.It("logs with default log level", func() {
				operatorConfig := frrk8sv1beta1.FRRK8sConfiguration{
					ObjectMeta: metav1.ObjectMeta{
						Name:      k8s.FRRK8sConfigurationName,
		...
		ginkgo.DescribeTable("when FRRConfiguration is added", func(logLevel string) {
			...
			err = updater.Update(nil, config)
			Expect(err).NotTo(HaveOccurred())
			checkFRRK8sLogs(cs, frrk8sPods, beforeUpdateTime, logLevel)
			checkBGPStateLogs(cs, frrk8sPods, beforeUpdateTime, logLevel)
		},
	...
	ginkgo.Context("for frr", func() {
		ginkgo.When("no logLevel configuration is present", func() {
			ginkgo.It("logs with default log level", func() {
				...
				checkFRRConfigLogLevel(cs, defaultLogLevel)
			})
		})
		...
		ginkgo.DescribeTable("logLevel configuration with logLevel is present", func(logLevel string) {
			...
			checkFRRConfigLogLevel(cs, logLevel)
		},
			ginkgo.Entry("logs with debug level when logLevel is set to debug", "debug"),
			ginkgo.Entry("logs with info level when logLevel is set to info", "info"),
		)
...
func checkFRRConfigLogLevel(cs clientset.Interface, logLevel string) {
