...
// l3IP returns the L3 host IP (without mask) for the named target.
func (s *EVPN) l3IP(name string) string {
...
// l3IPv6 returns the L3 IPv6 host IP (without mask) for the named
// target.
func (s *EVPN) l3IPv6(name string) string {
	ip, _, _ := net.ParseCIDR(fmt.Sprintf(s.cfg.L3IPv6PrefixFmt, s.indices[name]))
	return ip.String()
}

// L2IPForFamily returns the L2 access port IPs (without mask) for the named
// target on the provided IP families.
func (s *EVPN) L2IPForFamily(name string, family ipfamily.Family) []string {
	switch family {
	case ipfamily.IPv4:
		return []string{s.l2IP(name)}
	case ipfamily.IPv6:
		return []string{s.l2IPv6(name)}
	case ipfamily.DualStack:
		return []string{s.l2IP(name), s.l2IPv6(name)}
	}
	return nil
}

// L3PrefixForFamily returns the L3 VRF connected network prefixes (CIDR) for
// the named target on the provided IP families.
func (s *EVPN) L3PrefixForFamily(name string, family ipfamily.Family) []string {
	switch family {
	case ipfamily.IPv4:
		return []string{s.l3Prefix(name)}
	case ipfamily.IPv6:
		return []string{s.l3IPv6Prefix(name)}
	case ipfamily.DualStack:
		return []string{s.l3Prefix(name), s.l3IPv6Prefix(name)}
	}
	return nil
}

// L3IPForFamilyq returns the L3 host IPs (without mask) for the named target on
// the provided IP families.
func (s *EVPN) L3IPForFamily(name string, family ipfamily.Family) []string {
	switch family {
	case ipfamily.IPv4:
		return []string{s.l3IP(name)}
	case ipfamily.IPv6:
		return []string{s.l3IPv6(name)}
	case ipfamily.DualStack:
		return []string{s.l3IP(name), s.l3IPv6(name)}
	}
	return nil
}

// SetupEVPN configures EVPN Linux networking (bridge, VXLAN, VNI, VRF, access
// ports) on all cluster nodes and the provided external FRR container. It
// returns EVPN struct that provides name-based address lookups for each target.
func SetupEVPN(cs clientset.Interface, externalFRR *frrcontainer.FRR, cfg EVPNConfig) (*EVPN, error) {
	nodes, err := cs.CoreV1().Nodes().List(context.Background(), metav1.ListOptions{})
	if err != nil {
		return nil, err
	}
	if err := setupEVPNNetworking(cs, nodes.Items, externalFRR, cfg); err != nil {
		return nil, err
	}
	...
	indices[externalFRR.Name] = len(nodes.Items) + 1
	return &EVPN{cfg: cfg, indices: indices}, nil
}

// CleanupEVPN tears down EVPN Linux networking on all cluster nodes and the
// provided external FRR container.
func CleanupEVPN(cs clientset.Interface, externalFRR *frrcontainer.FRR, cfg EVPNConfig) error {
	nodes, err := cs.CoreV1().Nodes().List(context.Background(), metav1.ListOptions{})
	if err != nil {
		return err
	}
	return cleanupEVPNNetworking(cs, nodes.Items, externalFRR, cfg)
}
...
func setupEVPNNetworking(cs clientset.Interface, nodes []corev1.Node, externalFRR *frrcontainer.FRR, cfg EVPNConfig) error {
	...
	for i, node := range nodes {
		pod, err := k8s.PodForNode(pods, node.Name)
		if err != nil {
			return err
		}
