...
build: manifests generate fmt vet ## Build k8s-frr binary.
	go build -o bin/frr-k8s cmd/frr-k8s-controller/main.go
	go build -v -o bin/frr-metrics cmd/metrics/exporter.go
	go build -v -o bin/frr-status cmd/status/exporter.go
	go build -v -o bin/statuscleaner cmd/statuscleaner/main.go
